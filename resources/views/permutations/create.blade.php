@extends('layouts.app')
@section('content')
<form style="width: 22rem;" method="POST" action="{{route('permutations.store')}}" class="mt-5">
    <!-- Name input -->
    @csrf
    <div  class="form-outline mb-4">
        <label class="form-label" for="form5Example1">Date</label>
      <input type="date" name="date" value='{{old('date')}}' id="form5Example1" class="form-control" />
      @error('date')
    ...
    <div  class="form-outline mb-4">
      ...
      <input type="radio" name="valide" value='1' @if(old('valide') == 1) checked @endif id="form5Example1" style="width:15px;height: 18px;" class="p-4 mr-3" />
      <label class="form-label" for="form5Example1">Non</label>
      <input type="radio" name="valide" value='0' id="form5Example1" @if(old('valide') == 0) checked @endif style="width:15px;height: 18px;"class="p-4" />
      @error('valide')
      <div class="alert alert-danger m-3">
        {{ $message }}
    </div>
    ...
    <div class="form-outline mb-4">
        ...
        <select id="regionSelect" name="formateur_id" class="form-control">
            ...
            @if(old('formateur_id') == $formateur->id)
